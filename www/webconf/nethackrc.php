...
if (isset($_POST['action'])) {
	$udata = read_file(plr_rcfile($real_username), 0);
	...
	$fd=fopen(plr_rcfile($real_username), "w");
	fwrite($fd, $udata);
	fclose($fd);
...
if (!isset($_POST['action'])) {
   ...
   $user_rc_file = plr_rcfile($real_username);
	...
	echo "<P>Your <A HREF=\"".plr_rcfile_url($real_username)."\">current config file</A>.";
