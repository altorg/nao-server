...
use File::Tail;
use strict;
do "/opt/nhlog2mysql/XLogfile-4k.pm";

my $xlogfile = "/opt/nethack/hardfought.org/fourkdir/save/xlogfile";
my $fh;
my $line;
my $xlogline;
my $oneline;
...
sub insertxlogline
{
	...
	$dbh->do("insert into xlogfile (version,variant,points,deathdnum,deathlev,maxlvl,hp,maxhp,deaths,deathdate,birthdate,"
...
$dbh = DBI->connect("dbi:mysql:xlogfile_4k:localhost","nhdb","123456",$dbargs) or die("Unable to connect: $DBI::errstr\n");
print "Deleting all rows from current xlogfile_4k.db\n";
